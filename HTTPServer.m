...
- (void)performDefaultRequestHandling:(HTTPServerRequest *)mess {
    ...
    if ([method isEqual:@"GET"]) {
		...
		if ([[uri relativeString] hasPrefix:@"/slideshow-features"]) {
			...
			if (delegate && [delegate respondsToSelector:@selector(airplayDidAskDuration)]) { 
				_playDuration =  [delegate airplayDidAskDuration];
			}
			...
			bool bufferEmpty = (_playDuration+_playPosition == 0.0f);
			bool readyToPlay = !bufferEmpty;
			...
			NSString *resp = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n\
							  ...
							  </plist>", _playDuration, _playPosition, _playRate, bufferEmpty ? @"true" : @"false", readyToPlay ? @"true" : @"false", _playDuration, _playDuration];
		...
		else if ([[uri relativeString] hasPrefix:@"/scrub"]) {
			...
			NSString *resp = [NSString stringWithFormat:@"duration: %f\nposition: %f", _playDuration, _playPosition];
		...
		if ([[uri relativeString] hasPrefix:@"/reverse"]) {
			...
			NSString *seconds = [[uri relativeString] substringFromIndex:16];
			...
			_playPosition = [position intValue];
